-- Fix get_admin_stats(): cast level to text in BOTH subqueries before UNION ALL
-- to avoid "UNION types difficulty_enum and text cannot be matched" error
...
BEGIN
  ...
  -- Cast level to text in EACH branch of UNION ALL to avoid enum/text mismatch
  ...
  FROM (
    ...
    FROM (
      SELECT score, time_seconds, mistakes, level::text AS level FROM user_game_history
      UNION ALL
      SELECT score, time_seconds, mistakes, level::text AS level FROM guest_game_history
    ) combined
    GROUP BY level
    ORDER BY 
      CASE level
