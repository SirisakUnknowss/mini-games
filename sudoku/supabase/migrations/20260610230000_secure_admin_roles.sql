...
-- Enable RLS (only SELECT allowed, INSERT/UPDATE/DELETE are blocked for standard clients)
ALTER TABLE public.user_roles ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Allow select roles for logged in users"
  ON public.user_roles
  FOR SELECT
  TO authenticated
  USING (true);

-- 2. Update get_admin_stats() function to verify via the user_roles table
